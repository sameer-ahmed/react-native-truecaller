...
`npm install @mesameerahmed/react-native-truecaller --save` or `yarn add @mesameerahmed/react-native-truecaller`

- for linking both android and ios, run the following command
`
	react-native link
`
add this in header search path in buid settings in xcode `$(SRCROOT)/../node_modules/@mesameerahmed/react-native-truecaller/ios` - recursive
